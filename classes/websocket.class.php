...
class WSClient extends \WebSocket\Client {
    ...
    public function log($msg, $category = "debug") {
        Log::log($msg, $category, $this->sourcename == null ? get_class($this) : $this->sourcename);
    }
...
class Transitioner {
    ...
    public function run($sockettimeout = 5) {
        ...
        $this->log("all " . ($this->nRunnableAutos ? "but {$this->nRunnableAutos} " : "") . "automatons finished - end");
    }
...
class AppServiceLogin {
    ...
    private function getSessionkey() {
        ...
        if (($sk = $this->readSessionKey()) !== false) {
            $this->log("Using saved session keys", "runtime");
